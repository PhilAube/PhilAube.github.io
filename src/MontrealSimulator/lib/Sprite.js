...
export default class Sprite {
	...
	static generateSubsetFromSpriteSheet(spriteSheet, tileWidth, tileHeight, index) {
		const sprites = [];
		const sheetWidth = spriteSheet.width / tileWidth;
		const sheetHeight = spriteSheet.height / tileHeight;

		for (let x = 0; x < sheetWidth; x++) {
			sprites.push(new Sprite(spriteSheet, x * tileWidth, index * tileHeight, tileWidth, tileHeight));
		}
